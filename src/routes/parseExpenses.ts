...
function expenseLine(expense: any, ODnum: number) {
  ...
  return [
    ...
    expense["Donors"] +
      "/" +
      parseActivity(expense["Activity Number"]) +
      "/" +
      expense["Country"] + "/" +
      expense["Area"].padStart(2, "0"),
    "",
    "",
    expense["CHF Amount"],
  ];
...
function parseActivity(activityNumber: string) {
  //if activity number contains a '-' it's good to return as is

  if (activityNumber.includes("-") && activityNumber.length == 8) {
    return activityNumber;
  } else if (activityNumber.includes("-") && activityNumber.length == 6) {
    return "AC" + activityNumber;
  } else if (activityNumber.includes("-") && activityNumber.length == 5) {
    return "AC0" + activityNumber;
  } else if (activityNumber.includes("AC")) {
    return activityNumber + "-00";
  } else {
    return "AC" + activityNumber.padStart(3, "0") + "-00";
  }
}
...
export async function createExpenseOutput(
  data: any,
  lookupTable: any,
  filename: string,
  ODstring: string
